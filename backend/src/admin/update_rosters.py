...
def hide_all_ids() -> None:
    ...
    """
    _environment_chooser.operate_on_all_attendance(
        display.hide_columns, start_index=0, end_index=1
    )
...
def update_all_phone_numbers() -> None:
    ...
    update_roster_phone_numbers(
        spreadsheet_id=spreadsheet, asurite_to_phones=asurite_to_phones, overwrite=False
    )


def update_roster_phone_numbers(
    spreadsheet_id: str, *, asurite_to_phones: Dict[str, str], overwrite: bool = False
) -> None:
    """
    ...
    updated_phones = columns.update(
        updated_values=asurite_to_phones,
        grid=grid,
        key_index=0,
        target_index=phone_column_index,
        overwrite=overwrite,
    )
    sheet.update_values(spreadsheet_id, range_=range_, grid=updated_phones)
...
def convert_all_ids_to_asurite() -> None:
    ...
    """
    spreadsheet = (
        "1omms6ldwSZWgDXRf2HYDtGoCwzekfuSt7OAA8yVyRwY"
    )  # TODO(asurite): remove and use operate_on_all()
    values = sheet.get_values(file_ids.master, range_="Master!A2:D")
    id_to_asurite = {
        v[column_indexes.master["id"]]: v[column_indexes.master["asurite"]]
        for v in values
    }
...
def convert_roster_ids_to_asurite(
    spreadsheet_id: str, *, id_to_asurite: Dict[str, str]
) -> None:
    """
    ...
    range_ = "A2:A"
    grid = sheet.get_values(spreadsheet_id, range_=range_)
    asurites = columns.update(
        updated_values=id_to_asurite,
        grid=grid,
        key_index=0,
        target_index=0,
        overwrite=True,
