...
def main() -> None:
    ...
    if not override:
        create_legacy_tabs(
            include_committees=roster_targets.committees,
            include_mission_teams=roster_targets.mission_teams,
        )
    setup_semester.prepare_all_rosters(
        include_committees=roster_targets.committees,
        include_mission_teams=roster_targets.mission_teams,
        add_colors=False,
...
@command_line.log(
    start_message="Copying old rosters into legacy tabs.",
...
def create_legacy_tabs(
    *,
    ...
    def create(spreadsheet_id: str) -> None:
        ...
        grid = sheet.get_values(
            spreadsheet_id=spreadsheet_id, range_="C:Z", sheets_service=sheets_service
        )
        sheet.update_values(
            spreadsheet_id=spreadsheet_id,
            range_="'OLD DATA'!A:Z",
