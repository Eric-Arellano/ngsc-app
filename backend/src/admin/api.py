...
def api_create_file():
    ...
    arguments = generate_batch_arguments(
        payload=payload, parse_func=parse_create, drive_service=drive_service
    )
    create.batch(
        arguments=arguments,
...
def api_create_folder():
    ...
    create.batch_folder(
        arguments=arguments, drive_service=drive_service, include_output=False
    )
...
def api_copy_file():
    ...
    copy.batch(arguments=arguments, drive_service=drive_service, include_output=False)
...
def api_move_file():
    ...
    drive_service = drive_api.build_service()
    arguments = generate_batch_arguments(
        payload=payload, parse_func=parse_move, drive_service=drive_service
    )
...
def api_rename_file():
    ...
    drive_service = drive_api.build_service()
    arguments = generate_batch_arguments(
        payload=payload, parse_func=parse_rename, drive_service=drive_service
    )
...
def api_remove_file():
    ...
    arguments = generate_batch_arguments(
        payload=payload, parse_func=parse_remove, drive_service=drive_service
    )
...
def parse_global_source(
    payload: Dict, *, drive_service: discovery.Resource = None
    ...
    return find.recursive_resource(
        path=path.split("/"),
    ...
    *,
    ...
    return [
        create.BatchArgument(
            name=target["targetPath"].split("/")[-1],  # last element of path list
            parent_folder_id=find.parent_folder_for_path(
...
def parse_copy(
    *,
    ...
    parent_folder_ids: List[drive_api.ResourceID],
    drive_service: discovery.Resource = None,
) -> List[copy.BatchArgument]:
    return [
        copy.BatchArgument(
            origin_resource_id=original_resource_id,
            new_name=target["targetPath"].split("/")[-1],  # last element of path list
            target_folder_id=find.parent_folder_for_path(
                path=target["targetPath"],
                root_folder_id=folder_id,
                drive_service=drive_service,
            ),
...
def parse_remove(
    *,
    ...
    parent_folder_ids: List[drive_api.ResourceID],
    drive_service: discovery.Resource = None,
) -> List[copy.BatchArgument]:  # TODO: add Mime type
    return [
        remove.BatchArgument(
            resource_id=find.recursive_resource(
                path=target["targetPath"].split("/"),
...
def parse_rename(
    *,
    ...
    return [
        rename.BatchArgument(
            resource_id=find.recursive_resource(
                path=target["sourcePath"].split("/"),
                parent_folder_id=folder_id,
                drive_service=drive_service,
            ),
...
def parse_move(
    *,
    ...
    return [
        move.BatchArgument(
            origin_resource_id=find.recursive_resource(
                path=target["sourcePath"].split("/"),
                parent_folder_id=folder_id,
                drive_service=drive_service,
            ),
